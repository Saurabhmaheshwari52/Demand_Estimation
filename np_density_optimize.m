...
tail_size = mesh.m/8; %size enforce non-dec/non-inc 8
...
b=[zeros(mesh.m+1,1)];
b=[zeros(mesh.m+1,1);zeros(tail_size,1);zeros(tail_size,1);ones(mesh.m-1,1)*smoothn/mesh.m;ones(mesh.m-1,1)*smoothn/mesh.m];
...
%  C1=[C]; d1=[d];
 C1=[C;0.001*mat_sparse]; d1=[d;zeros(mesh.m,1)];
% 
